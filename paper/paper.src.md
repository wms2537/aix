...
An LLM agent that edits a spreadsheet or a dbt project produces an artifact that
*opens fine* but may be silently wrong: a structural edit that fails to propagate
references corrupts computed values with no visible symptom, and neither the agent
nor a reviewer can see it without the defining engine. We ask which such edits can
be certified **engine-free** — offline, against the artifact's own structure,
trusting neither the editor nor a recomputation — and answer by machine-checking
both sides of the boundary (Lean 4, no `sorry`, axioms `propext`/`Quot.sound`
only). Relabeling edits are certifiable: their premise is decidable from syntax,
and our executable decision procedure `check` carries a soundness theorem
quantifying over **every possible engine**. Edits introducing unwitnessed semantics
are uncertifiable: indistinguishable engines disagree, so certify-or-refuse is the
only sound shape. The middle ground — copies of witnessed formulas — is structured
by an argument-value witnessing criterion with theorems on both of its sides.

The theory is load-bearing in the running system. The deployed checker agrees with
the Lean decider; the trusted byte→token layer carries a **verified reference
tokenizer** (losslessness, literal opacity, σ-image) held to the production
tokenizer by a {{diff_requests}}-comparison corpus differential with zero
disagreements on its model surface ({{diff_insurface_pct}} of comparisons; the
largest remainder, ASCII-sheet-qualified formulas, is disclosed-unverified
surface); and a production certifier covers five structural operations.

Two pre-registered, run-once **locked tests** on external corpora (EUSES, Enron,
production dbt projects) corroborate the theory: **zero false certifications
across {{combined_foreign_calls}} foreign-edit calls**, zero mismatches across
...
3. **(Mechanism, theory-linked down to bytes.)** A certify-or-refuse router whose
   ...
   **verified reference tokenizer** for the trusted byte→token layer (losslessness,
   opacity, σ-image; Theorem 7), held to the production tokenizer by a corpus-scale
   differential — zero disagreements on the model surface, which covers
   {{diff_insurface_pct}} of comparisons; the largest remainder
   ({{diff_ascii_pct}}, ASCII-sheet-qualified formulas) is DISCLOSED-UNVERIFIED
   surface that production shifts outside the model, with fail-closed refusal for
...
discharged empirically: a corpus-scale differential ({{corpus_formulas}} unique real corpus
formulas + {{diff_battery}} synthetic battery formulas, × four edits = **{{diff_requests}} comparisons**)
between the executable Lean reference and the production tokenizer agrees
**{{diff_insurface_agree}} / {{diff_insurface_agree}} in-surface and {{diff_guard_agree}} / {{diff_guard_agree}} on guard refusals — zero
disagreements**, after itself finding a third real production defect (§6). The
compared surface is {{diff_insurface_pct}} of all comparisons; the remainder is counted and
classified, not compared: {{diff_ascii_qualified}} ({{diff_ascii_pct}}) ASCII-sheet-qualified formulas — which
production shifts, an *unverified-by-this-model* surface we disclose rather than
claim — plus {{diff_deleterange}} delete-clamp range cases and {{diff_wholecolrow}} whole-column/row references.
...
of which **147/172 = {{ab_confirmed_pct}} is confirmed-genuine reference corruption** after our own
...
cross-corpus generality claims are correspondingly tempered. The third corpus is
one **real production dbt project** (Mattermost's data warehouse,
{{dbt_mattermost_models}} models). The dbt leg's
...
- **Reference-shift arithmetic: 0 errors in {{v1_cellchecks_total}} real formula cells** across the
  four ops (Enron: {{v1_enron_cellchecks}} cells, 100%). The checked-cell denominator is bounded by
  ...
  (0.034% of the {{v1_euses_cellchecks}} EUSES-leg cells), all in one Japanese workbook — was **not** a shift error: xlq
...
- **Fail-closed cost, measured:** the full guarded pipeline (transform residual ∪
  own-certify not certified) refuses {{v1_euses_cost_pct}} (EUSES) and {{v1_enron_cost_pct}} (Enron) of eligible
...
- **dbt (production project):** mini-adapter parse coverage {{dbt_mattermost_coverage_pct}} (*above* the
  predicted <30% — a favorable disconfirmation; 152 models with unmodeled Jinja fail
  closed), and on the {{dbt_mattermost_closed}}-model closed subgraph the certify legs behaved exactly as
...
corpus included the v1 locked corpora, and {{euses_byte_identical}} of the 500 EUSES shift-leg files are
byte-identical v1 copies. The Enron leg is the **less-contaminated headline** ({{v2_enron_cellchecks}} cell-checks, zero
mismatches): {{enron_source_overlap}} of its 362 eligible files ({{enron_source_overlap_pct}}) are re-conversions of source
workbooks whose v1 conversions fed the discovery corpus — zero byte-identical, the
overlap is at source-document level — versus {{euses_byte_identical}}/500 ({{euses_byte_identical_pct}}) byte-identical v1 copies
...
with the code under test, breaking independence on exactly that class (6 of {{corpus_formulas}}
...
- **The probabilistic tier collapses in the tail**: the random Enron sample
  ...
  optimization.
- **The fail-closed cost is structural and levered**: {{v2_euses_cost_pct}} (EUSES cap sample) and {{v2_enron_cost_pct}}
...
- **Two-model agent study** (21 tasks — the corpus's honest ceiling — × fast/mid
  ...
  deterministic synthetic perfect-agent arm: its refusals fell **{{smoke_prefix_refusals}} → {{smoke_postfix_refusals}}** pre-fix to
...
86.6% to {{ab_confirmed_pct}} confirmed-genuine and record the mislabel here. Most recently, the
...
`SUM` by accident, wrong for `COUNT`-class functions (3 disagreements in the full
differential of Theorem 7, all this shape). Fixed with regressions; the post-fix differential
agrees everywhere on the model surface; and the fix *measurably reduced the guard's
cost*: the like-for-like synthetic arm's refusals of correct work fell {{smoke_prefix_refusals}} → {{smoke_postfix_refusals}} —
